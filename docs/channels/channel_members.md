...
> [!NOTE]
> You can familiarize yourself with all permissions in the [Permissions section](/chat/docs/ruby/chat_permission_policies/).
...
```ruby
cutoff = DateTime.now - 7 # Last 7 days
...
response = channel.update_member_partial(user_id, set: { 'color' => 'red' }, unset: ['hat'])
```
