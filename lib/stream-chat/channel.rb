...
module StreamChat
  class Channel
    ...
    sig { returns(T::Array[StringKeyDictionary]) }
    attr_reader :members

    sig { params(client: Client, channel_type: String, channel_id: T.nilable(String), custom_data: T.nilable(StringKeyDictionary)).void }
    def initialize(client, channel_type, channel_id = nil, custom_data = nil)
      @channel_type = channel_type
      @id = channel_id
      @cid = T.let("#{@channel_type}:#{@id}", String)
      @client = client
      @custom_data = T.let(custom_data || {}, StringKeyDictionary)
      @members = T.let([], T::Array[StringKeyDictionary])
    end
    ...
    sig { params(message: StringKeyDictionary, user_id: String).returns(StreamChat::StreamResponse) }
    def send_message(message, user_id)
      payload = { message: add_user_id(message, user_id) }
      @client.post("#{url}/message", data: payload)
    end

    sig { params(event: StringKeyDictionary, user_id: String).returns(StreamChat::StreamResponse) }
    def send_event(event, user_id)
      payload = { 'event' => add_user_id(event, user_id) }
      @client.post("#{url}/event", data: payload)
    end

    sig { params(message_id: String, reaction: StringKeyDictionary, user_id: String).returns(StreamChat::StreamResponse) }
    ...
    sig { params(channel_data: T.nilable(StringKeyDictionary), update_message: T.nilable(StringKeyDictionary), options: T.untyped).returns(StreamChat::StreamResponse) }
    def update(channel_data, update_message = nil, **options)
      payload = { data: channel_data, message: update_message }.merge(options)
      @client.post(url, data: payload)
    end

    sig { params(set: T.nilable(StringKeyDictionary), unset: T.nilable(T::Array[String])).returns(StreamChat::StreamResponse) }
    ...
    sig { params(members: T::Array[StringKeyDictionary], message: T.nilable(StringKeyDictionary)).returns(StreamChat::StreamResponse) }
    ...
    sig { params(user_id: String, options: StringKeyDictionary).returns(StreamChat::StreamResponse) }
    ...
    sig { params(url: String, user: StringKeyDictionary, content_type: T.nilable(String)).returns(StreamChat::StreamResponse) }
    ...
    sig { params(payload: StringKeyDictionary, user_id: String).returns(StringKeyDictionary) }
